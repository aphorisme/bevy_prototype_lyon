...
/// Contains path data that can be used to get a `SpriteBundles` bundle with
/// a custom shape. Check out [`PathBuilder`](PathBuilder) to construct it.
pub struct Path(path::Path);

impl Path {
    /// Returns a `SpriteBundles` with the filled path as the mesh.
    pub fn fill(
        &self,
    ...
    /// Returns a `SpriteBundles` with the stroked path as the mesh.
    pub fn stroke(
        &self,
        material: Handle<ColorMaterial>,
        meshes: &mut ResMut<Assets<Mesh>>,
